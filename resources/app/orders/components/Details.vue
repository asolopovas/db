        ...
        <div class="max-w-[1004px] mx-auto shadow-lg">
            <div class="flex flex-wrap bg-stone-200 mx-auto mt-4 py-4">
                ...
                <div class="flex flex-col gap-3 items-start w-full p-4">
                    ...
                    <!-- Order  Inputs  -->
                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3 mb-3 w-full">
                    ...
                    <div class="flex flex-col w-full">
                        <hr />
                        <div class="order-controls grid md:grid-cols-2 lg:grid-cols-4 gap-2">
                            <base-button
...
<script setup lang="ts">
    ...
    const authorizeXero = async () => {
        const { data } = await apiFetch<{ redirect_url: string }>("/api/xero-connect")
        const authWindow = window.open(data.redirect_url, "_blank", "width=800,height=600")
        if (!authWindow) throw new Error("Popup blocked")
    }

    function issueOnXero() {
        if (!alerts.value.length) {
            loading.value = true
            store
                .dispatch("issueOnXero", { orderId: store.state.order.id })
    ...
    const downloadInvoice = async () => {
        try {
            const { data } = await apiFetch<Blob>(`/api/orders/${store.state.order.id}/download`, {
                method: "GET",
                responseType: "blob",
            })
