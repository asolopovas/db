<template>
  <tr>
    ...
    <td class="t-col t-col-action" data-label="Action">
      <div class="flex gap-2 items-center justify-end md:flex text-neutral-50">
        ...
        <base-button class="btn-action bg-blue-600" @click="download">
          <DownloadIcon />
        </base-button>
        <base-button class="btn-action bg-green-600" @click="edit(item.id)">
          <EditIcon />
        </base-button>
        <base-button class="btn-action bg-indigo-600" @click="duplicate">
          <CopyIcon />
        </base-button>
      </div>
    </td>
...
const download = async () => {
  try {
    ...
    link.setAttribute(
      "download",
      `Customer - ${item.customer} #${item.id}.pdf`,
    );
...
const duplicate = async () => {
  try {
    const response = await apiFetch(`/api/orders/${item.id}/duplicate`, {
      method: "POST",
    });
    const newId =
