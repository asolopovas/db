...
export function setXeroAuth(state: RootState, { authorized, expires_at }: { authorized: boolean; expires_at: string | null }) {
    state.auth.xero.authorized = authorized
    state.auth.xero.expires_at = expires_at
}
