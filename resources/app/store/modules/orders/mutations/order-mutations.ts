...
function removeEl<K extends keyof OrderState>(
  ...
  if (Array.isArray(target) && el && typeof el === "object" && "id" in el) {
    const index = target.findIndex(
      (item: Identifiable) => item.id === (el as Identifiable).id,
    );
    if (index !== -1) {
...
function resetEl<K extends keyof OrderState>(state: OrderState, loc: K): void {
  const target = state[loc];
...
function syncDiscountAmount(state: OrderState): void {
  ...
  state.discount = Number(
    ((subtotal * (state.discount_percent || 0)) / 100).toFixed(2),
  );
}
...
function updateBalance(state: OrderState): void {
  const validTaxDeductions = (state.tax_deductions || []).filter(
    (deduction): deduction is TaxDeduction & { id: number } =>
